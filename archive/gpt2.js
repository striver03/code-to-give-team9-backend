...
const getnextQues = async (req, res) => {
  ...
  if(!prevData || !isModifiable) //when first question or isModifiable false
  {
    // console.log("hello");
    let p = `Translate this question's text and its options(if any) to the language specified in the language Key , question in JSON fommat is : ${JSON.stringify(curr)}`;
    ...
    if(!isModifiable && prevData) 
    {
      //This prompt is for adding the data of previous questions.
      let p = `I'm currently providing you the question with it's response of the user. Store it in your memory to make the questions asked in future more aligned to user's mindset. You don't need to modify this question, just collect the information about the user.
      ${JSON.stringify(prevData)}`;

      history.push({role:"user",content: p});
    }

    return res.status(200).send({modifiedQues:response});
  }
    ...
    //   const response = JSON.parse(resp.data.choices[0].message.content);
      ...
      else res.status(200).json({modifiedQues:resp.data.choices[0].message.content});
...
const getQuesPrompt = (curr) => {
  let prompt = `I'm giving you the question in JSON format. Based on the things you know about the user, personalize the question and return that in JSON format. Make sure you do not change the pronounce and context of the question. The question should be framed keeping in mind the type and options (if any) of the actual question and should not change the context of the actual question.The question is in JSON format and give the resonse in the similar format (keep options same though) and JUST GIVE THE RESPONSE IN JSON WITHOUT ANY OTHER ADDED EXPLANATION TRANSLATE THIS QUATIONS's TEXT AND ITS OPTIONS(IF ANY) TO THE LANGUAGE SPECIFIED IN THE LANGUAGE KEY-  ${JSON.stringify(curr)}`;
  return prompt;
};
